...
import org.security.services.jwt.JwtConfigure;
 ...
 *  JWt token is used in order to authenticate user.
 *  Cookies is disabled.
 *  Custom filter is used to apply JWT base authentication.
 *
 */
@Configuration
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(final HttpSecurity http) throws Exception {
        http.httpBasic().disable().csrf().disable().sessionManagement()
                ...
                .authenticated().and().apply(new JwtConfigure(jwtTokenProvider));
    }
