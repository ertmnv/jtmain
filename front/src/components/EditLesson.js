...
export class EditLesson extends Component {
  ...
  onSubmit = (lesson) => {
    this.props.editLesson({...lesson, id:this.props.lesson.id}, this.props.course.id, this.props.history);
  };
...
const mapStateToProps = (state, props) => {
    return {
        ...
        lesson: state.sections.sectionsByCourse
            ...
            .lessonDtoList.find(lesson => lesson.id == props.match.params.lessonId)
    }
}

const mapDispatchToProps = (dispatch) => ({
    editLesson: (section, courseId, history) => dispatch(editLesson(section, courseId, history))
});
